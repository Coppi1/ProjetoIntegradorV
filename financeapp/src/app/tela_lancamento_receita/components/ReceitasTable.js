...
export default function ReceitasTable() {
    ...
    useEffect(() => {
        axios.get('http://localhost:4000/receitas')
            .then(response => {
    ...
    const renderEditButton = (rowData) => {
        return (
            <Button icon="pi pi-pencil" className="p-button-rounded p-button-success p-mr-2" onClick={() => editReceita(rowData)} />
        );
    };
            ...
            <div className="p-d-flex p-jc-between p-mb-2">
                <div className="p-d-flex p-ai-center">
                    <span className="p-input-icon-left">
                        ...
                        <InputText value={globalFilter} onChange={(e) => setGlobalFilter(e.target.value)} placeholder="Pesquisar por nome" />
                    </span>
                </div>
            </div>
            <DataTable className="custom-data-table" value={receitas} globalFilter={globalFilter} globalFilterMode="contains"
                paginator rows={10} emptyMessage="Nenhuma receita encontrada">
